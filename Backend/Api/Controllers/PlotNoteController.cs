...
{
    [ApiController]
    [Route("api/projects/{projectId}/plot-notes")]
    [Authorize]
    public class PlotNoteController : AppControllerBase
    {
        private readonly IPlotNoteService _service;

        public PlotNoteController(IPlotNoteService service)
        {
            _service = service;
        }
        ...
        [HttpPost]
        public async Task<IActionResult> CreateEntry(Guid projectId, [FromBody] CreatePlotNoteRequest request)
        {
            var userId = GetUserId();
            if (userId == null) return Unauthorized();
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> UpdateEntry(Guid projectId, Guid id, [FromBody] UpdatePlotNoteRequest request)
        {
            try
            {
                var userId = GetUserId();
