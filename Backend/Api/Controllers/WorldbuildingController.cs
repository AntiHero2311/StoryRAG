...
namespace Api.Controllers
{
    ...
    public class WorldbuildingController : ControllerBase
    {
        ...
        public WorldbuildingController(IWorldbuildingService service)
        {
            _service = service;
        }
        ...
        public async Task<IActionResult> GetById(Guid projectId, Guid id)
        {
            try
            {
                ...
                if (result == null) return NotFound(new { message = "Không tìm thấy mục worldbuilding." });
                return Ok(result);
            }
            catch (KeyNotFoundException ex) { return NotFound(new { message = ex.Message }); }
            catch (Exception ex) { return StatusCode(500, new { message = ex.Message }); }
        ...
        [HttpPost]
        public async Task<IActionResult> Create(Guid projectId, [FromBody] CreateWorldbuildingRequest request)
        {
            try
            {
                var userId = GetUserId();
        ...
        [HttpPut("{id:guid}")]
        public async Task<IActionResult> Update(Guid projectId, Guid id, [FromBody] UpdateWorldbuildingRequest request)
        {
            try
            {
                var userId = GetUserId();
