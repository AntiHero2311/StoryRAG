...
namespace Service.Implementations
{
    public class ChapterService : IChapterService
    {
        ...
        public async Task<ChapterDetailResponse> GetChapterDetailAsync(Guid chapterId, Guid userId)
        {
            ...
            // Decrypt current version content
            string? content = null;
        ...
        public async Task<ChapterDetailResponse> CreateChapterAsync(Guid projectId, Guid userId, CreateChapterRequest request)
        {
            ...
            await _context.SaveChangesAsync(); // Need Id for version FK
            ...
            var version = new ChapterVersion
            {
                ...
                ChangeNote = string.IsNullOrEmpty(request.ChangeNote)
                    ? null
                    : EncryptionHelper.EncryptWithMasterKey(request.ChangeNote, rawDek),
                WordCount = wordCount,
                TokenCount = _chunkingService.EstimateTokenCount(request.Content),
                CreatedBy = userId,
            };
            _context.ChapterVersions.Add(version);
            await _context.SaveChangesAsync(); // Need version Id

            // Point chapter to first version
        ...
        {
            // Update = save new version automatically
            var saveRequest = new SaveNewVersionRequest
            {
                Content = request.Content,
                ChangeNote = request.ChangeNote
            };
            var result = await SaveNewVersionAsync(chapterId, userId, saveRequest);
        ...
        public async Task<ChapterVersionDetailResponse> GetVersionDetailAsync(Guid chapterId, int versionNumber, Guid userId)
        {
            ...
            var detail = MapToVersionDetailResponse(version, rawDek);
            return detail;
        }

        public async Task<ChapterDetailResponse> SaveNewVersionAsync(Guid chapterId, Guid userId, SaveNewVersionRequest request)
        {
            var chapter = await GetChapterWithOwnerCheckAsync(chapterId, userId);
            var rawDek = await GetRawDekAsync(userId);
        ...
        public async Task<ChapterDetailResponse> RestoreVersionAsync(Guid chapterId, int versionNumber, Guid userId)
        {
            var chapter = await GetChapterWithOwnerCheckAsync(chapterId, userId);
            var rawDek = await GetRawDekAsync(userId);
            ...
            // Restore = create a new version with content of target version
            string decryptedContent = EncryptionHelper.DecryptWithMasterKey(targetVersion.Content, rawDek);
            int newVersionNum = chapter.CurrentVersionNum + 1;

            var restoredVersion = new ChapterVersion
            {
                ChapterId = chapterId,
                VersionNumber = newVersionNum,
                Content = targetVersion.Content, // Already encrypted, reuse
                ChangeNote = EncryptionHelper.EncryptWithMasterKey($"Phục hồi từ phiên bản {versionNumber}", rawDek),
                WordCount = targetVersion.WordCount,
                TokenCount = targetVersion.TokenCount,
                CreatedBy = userId,
            };
            _context.ChapterVersions.Add(restoredVersion);
            await _context.SaveChangesAsync();

            chapter.CurrentVersionId = restoredVersion.Id;
            chapter.CurrentVersionNum = newVersionNum;
            chapter.WordCount = targetVersion.WordCount;
            chapter.UpdatedAt = DateTime.UtcNow;
            await _context.SaveChangesAsync();

            var detail = MapToDetailResponse(chapter, decryptedContent);
            detail.Versions = new List<ChapterVersionSummary> { MapToVersionSummary(restoredVersion, rawDek) };
            return detail;
        }
        ...
        public async Task<ChapterVersionDetailResponse> ChunkVersionAsync(Guid chapterId, Guid userId)
        {
            ...
            // Remove old chunks of this version
            _context.ChapterChunks.RemoveRange(version.Chunks);

            // Decrypt → split → encrypt each chunk → save
        ...
        private static ChapterVersionSummary MapToVersionSummary(ChapterVersion v, string rawDek) => new()
        {
            ...
            ChangeNote = string.IsNullOrEmpty(v.ChangeNote)
                ? null
                : EncryptionHelper.DecryptWithMasterKey(v.ChangeNote, rawDek),
