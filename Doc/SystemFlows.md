...
5. **Auto-Repair RAG trích dẫn & Mở rộng bối cảnh (RAG Quote Matching & Context Expansion):**
   - **Sửa lỗi lệch chương**: Thực hiện quét so khớp trực tiếp bằng cách tách trích dẫn `evidence` của AI thành các đoạn ngắn hơn qua dấu ba chấm (`...`, `..`, `…`) hoặc ký tự xuống dòng. Sau đó sử dụng hàm chuẩn hóa văn bản `NormalizeForMatching` (bỏ qua dấu câu, dấu ngoặc và khoảng trắng thừa) để đối chiếu chính xác với toàn bộ bản thảo đã giải mã trên Backend. Hệ thống sẽ ưu tiên chọn các chunk có tỷ lệ khớp các đoạn trích cao nhất, triệt tiêu lỗi lệch chương.
   - **Bôi màu thông minh ở Frontend**: Khi mở giao diện "Tra cứu đoạn văn bản gốc", Frontend tách dẫn chứng theo dấu ba chấm/xuống dòng và tìm kiếm chỉ số của các mảnh khớp độc lập trong văn bản gốc của chunk để bọc thẻ `<mark>` bôi vàng, đảm bảo hiển thị đầy đủ ngữ cảnh đối chứng mà không bị mất dấu hay chỉ hiển thị một câu đơn lẻ.
...
6. **Vẽ biểu đồ Nhịp độ & Cảm xúc (Narrative Analytics - Task 3):**
   ...
   - **Thẩm mỹ Đồ họa & Bố cục mới**: Các biểu đồ được xếp chồng dọc (vertical stacked) để giãn rộng không gian ("cho to biểu đồ ra"), đồng thời tích hợp thanh cuộn ngang (`overflow-x-auto`) để tránh dồn ép trục X khi truyện có nhiều chương (ví dụ 100 chương). Đường cong vẽ bằng dạng mềm mại (cubic bezier curve) tích hợp SVG dynamic tooltips và vertical crosshairs.
