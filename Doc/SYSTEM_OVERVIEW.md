...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IAuthService`          | Đăng ký, đăng nhập (email & Google), refresh token, đổi mật khẩu, quên/đặt lại mật khẩu. |
| `IUserService`          | Xem và cập nhật thông tin hồ sơ cá nhân người dùng (profile). |
| `IUserSettingsService`  | Cài đặt editor của người dùng (font chữ, kích thước chữ). |
| `IProjectService`       | CRUD dự án truyện, thống kê dashboard tác giả, xuất toàn bộ dự án truyện. |
| `IChapterService`       | CRUD chương truyện, quản lý phiên bản chương (tạo, chuyển đổi, ghim chống tự động dọn dẹp, xóa), thực hiện chunk nội dung chương. |
| `IExportService`        | Xuất dự án truyện hoặc chương cụ thể ra định dạng file `.docx`, `.txt`, `.pdf`. |
| `IPaymentService`       | Quản lý thanh toán gói dịch vụ qua cổng VNPay, lưu lịch sử giao dịch và cập nhật trạng thái hóa đơn. |
| `IGenreService`         | Quản lý danh sách thể loại truyện (CRUD dành cho Admin). |
| `ISubscriptionService`  | Quản lý các gói dịch vụ (Free/Basic/Pro/Enterprise), đăng ký/gia hạn gói và kiểm tra giới hạn sử dụng (token/số lượt phân tích) của người dùng. |
| `IAiChatService`        | Thực hiện chat RAG với AI dựa trên ngữ cảnh được truy hồi từ bản thảo chương đang viết (chỉ tính/khấu trừ token AI, không trừ lượt phân tích). |
| `IAiAnalysisHistoryService` | Lưu và truy xuất lịch sử phân tích chuyên sâu (cảnh, cliffhanger) của AI. |
| `IEmbeddingService`     | Giao tiếp với API Gemini để chuyển đổi các đoạn văn bản thành vector embedding 768 chiều. |
| `IChunkingService`      | Chia nhỏ nội dung chương thành các phần (chunks) kích thước ~1500 ký tự với overlap ~150 ký tự để tối ưu hóa truy hồi RAG. |
| `IAiWritingService`     | Dịch vụ hỗ trợ viết của AI, bao gồm viết lại đoạn văn (rewrite), phân tích bối cảnh/cliffhanger/ba hồi dựa trên RAG. |
| `IProjectReportService` | Thực hiện phân tích & chấm điểm toàn bộ truyện theo Rubric 5 điểm (20 tiêu chí), Zero Hallucination, phát hiện 6 cảnh báo đặc biệt (INCOMPLETE / REPETITION / PLAGIARISM_RISK / INCONSISTENCY / SEXUAL_CONTENT / ANTI_STATE), tự động trích xuất dữ liệu Story Bible (nhân vật, thế giới, chủ đề, cốt truyện) lưu thành snapshot (`ProjectAnalysisFacts`). |
| `IProjectAnalysisJobService` | Quản lý hàng đợi và tiến trình chạy các job phân tích truyện bất đồng bộ, cập nhật tiến độ (progress), hủy job và lấy kết quả báo cáo. |
| `INarrativeAnalyticsService` | Sinh dữ liệu biểu đồ nhịp độ kể chuyện (pacing) và dòng cảm xúc (emotion) của tác phẩm. |
| `IReportExportService` | Hỗ trợ xuất báo cáo phân tích truyện (ProjectReport) ra định dạng file PDF. |
| `IEmailService`         | Gửi email thông báo, chào mừng và liên kết đặt lại mật khẩu qua Gmail SMTP sử dụng MailKit. |
| `IAdminService`         | Cung cấp số liệu thống kê tổng quan hệ thống (users, projects, reports, revenue) cho dashboard quản trị của Admin. |
| `IBugReportService`     | CRUD báo cáo lỗi từ người dùng, cập nhật trạng thái xử lý lỗi của Staff/Admin. |
| `IStaffService`         | Quản lý phản hồi chuyên môn (StaffFeedbacks) của Staff gửi cho tác giả, quản lý FAQ và mẹo viết truyện (writing tips). |
| `IAnalysisJobCancellationRegistry` | Đăng ký, theo dõi và quản lý việc yêu cầu hủy bỏ (RequestCancellation) các job phân tích AI đang chạy thông qua CancellationToken. |
| `IAnalysisJobQueue`     | Hàng đợi lưu trữ và phân phối các job phân tích AI bất đồng bộ theo mức độ ưu tiên của gói Subscription. |
| `INotificationService`  | Quản lý thông báo trong hệ thống, cho phép tạo thông báo cho user cụ thể hoặc hàng loạt user theo vai trò (roles), đánh dấu đã đọc. |
| `IProjectImportService` | Import bản thảo từ file tải lên (docx, txt), tự động nhận diện và tách thành các chương truyện, hỗ trợ trích xuất lại thông tin ban đầu. |
| `IStaffModerationService` | Cung cấp các công cụ kiểm duyệt cho Staff, bao gồm cảnh báo tác giả, tạm khóa dự án, đề xuất ban người dùng và xem hiệu suất làm việc của Staff. |
| `ISystemAuditLogService` | Ghi nhật ký hoạt động hệ thống (SystemLogs) để phục vụ cho mục đích bảo mật, kiểm toán và giám sát hành vi. |
| `ISystemConfigService`  | Quản lý cấu hình động hệ thống (bao gồm tham số RAG, Gemini API limits) có bộ nhớ đệm (cache), cho phép cập nhật trực tiếp không cần khởi động lại. |
