...
function statusBadge(status: string) {
  ...
  if (s === 'completed') return { label: 'Completed', icon: CheckCircle2, bg: 'bg-emerald-500/15', text: 'text-emerald-400', border: 'border-emerald-500/30' };
  if (s === 'cancelled') return { label: 'Cancelled', icon: XCircle, bg: 'bg-zinc-500/15', text: 'text-zinc-400', border: 'border-zinc-500/30' };
...
export default function StaffAnalysisJobsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await analysisJobService.getAnalysisJobs(statusFilter);
      setItems(data);
    } catch (err: unknown) {
      const message =
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-[var(--border-color)]">
                    {items.map(j => {
                      ...
                      const isStale = (j.status || '').toLowerCase() === 'processing' && j.last_heartbeat && (new Date().getTime() - new Date(j.last_heartbeat).getTime() > 15 * 60 * 1000);
                      const canRerun = isFailed || isStale;
                      ...
                      return (
                        <tr key={j.id} className="hover:bg-[var(--text-primary)]/3 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <p className="text-[var(--text-primary)] text-xs font-semibold">{j.project_title || '—'}</p>
                            <p className="text-[var(--text-secondary)] text-[10px] font-mono mt-0.5">{j.project_id}</p>
                          </td>
                          <td className="px-4 py-3">
                            <p className="text-[var(--text-primary)] text-xs font-semibold">{j.requested_by_name || '—'}</p>
                            <p className="text-[var(--text-secondary)] text-[10px] font-mono mt-0.5">{j.requested_by}</p>
                          </td>
                          <td className="px-4 py-3">
                          ...
                          <td className="px-4 py-3">
                            {canRerun && (
                              <button
                                ...
                                className="h-8 px-3 rounded-xl text-xs font-semibold transition-colors disabled:opacity-60 inline-flex items-center gap-1.5"
